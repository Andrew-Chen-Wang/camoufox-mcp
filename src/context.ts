...
`);
  ...
  async browserContext(): Promise<playwright.BrowserContext> {
    const { browserContext } = await this._ensureBrowserContext();
  ...
  async addRouteRule(pattern: string, handler: (route: playwright.Route) => Promise<void>) {
    const rule: RouteRule = { pattern, handler };
  ...
  listRouteRules(): string[] {
    return this._routeRules.map(rule => rule.pattern);
  }

  async removeRouteRule(pattern?: string) {
  ...
  async stopTracing(filename?: string) {
    const browserContext = await this.browserContext();
    if (!this._tracingActive)
